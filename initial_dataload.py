...
def import_asset(code: str, start_date: datetime.datetime, end_date: datetime.datetime) -> bool:
    ...
    asset = tracked_asset.TrackedAsset(symbol=code, date=latest_date,
    ...
    dates = [candle.timestamp.replace(hour=0, minute=0, second=0, microsecond=0) for candle in bars]
    ...
    asset.calculate_ema_big_long(prices=prices, dates=dates, db_client=mongo_client)
...
for symbol in symbols:
    if not import_asset(code=symbol, start_date=starting_datetime, end_date=today):
